using System.Collections.Generic;
using UnityEngine;

namespace SpaceShooter.Mono
{
    public class ObjectPool : Queue<GameObject>
    {
// PROPERTIES

        public int Max { get; private set; } = 0;
        public int Growth { get; set; } = 1;
        public GameObject Prefab { get; private set; } = null;

// INITIALISATION

        public ObjectPool(GameObject prefab, int max) : base(max){
            Prefab = prefab;
            Max    = max;
        }

        public ObjectPool(GameObject prefab, int growth, int max) : base(max){
            Prefab = prefab;
            Growth = growth;
            Max    = max;
        }

// POOLING

        public GameObject Get(){
            var next = TryGet();

            if (next != null)
                return next;

            Populate(Growth);
            return TryGet();
        }

        public GameObject TryGet(){
            GameObject next = null;
            int tries = 0;

            while (next == null && tries++ < base.Count){
                if (!base.TryDequeue(out next)) continue;
                if (next == null) continue;

                if (!next.activeSelf){
                    next.SetActive(true);

                    base.Enqueue(next);
                    return next;
                }
                base.Enqueue(next);
                next = null;
            }
            return null;
        }

        public void Populate(){
            Populate(Growth);
        }

        public void Populate(int amount){
            amount = Mathf.Clamp(
                amount, 0, this.Max - base.Count
            );
            for (int i = 0; i < amount; i++){
                var component = GameObject.Instantiate(Prefab);
                    component.SetActive(false);

                base.Enqueue(component);
            }
        }

    }
}